...
class OneStepDataset(pyg.data.Dataset):
    def __init__(self, data_path, split, window_length=7, noise_std=0.0, return_pos=False):
        ...
        # Load position data
        self.positions, self.dates = parse_position_data("/home/admin1/Desktop/GNNtrain2.txt")
        
        # Set metadata
        self.metadata = {
            "bounds": [[0, 1], [0, 1]],  # Normalized coordinates
            "default_connectivity_radius": 0.08,
            "vel_mean": [0, 0],
            "vel_std": [0.01, 0.01],
            "acc_mean": [0, 0], 
            "acc_std": [0.01, 0.01]
        ...
        # Create particle types (all same type in this case)
        self.particle_type = np.zeros(207, dtype=np.int64)
        ...
        # Create windows
        self.windows = []
        for i in range(len(self.positions) - window_length + 1):
            self.windows.append({
                "start_idx": i,
                "size": 207  # Number of particles
            })
    ...
    def get(self, idx):
        ...
        # Get position sequence for this window
        position_seq = torch.tensor(
            self.positions[start_idx:start_idx+self.window_length],
            dtype=torch.float32
        ).transpose(0, 1)  # [n_particles, window_length, 2]
        
        particle_type = torch.tensor(self.particle_type)
        
        # Process through existing preprocess function
        with torch.no_grad():
            graph = preprocess(
                particle_type,
                position_seq[:, :-1],  # All but last frame
                position_seq[:, -1],   # Last frame as target
                self.metadata,
                self.noise_std
            )
